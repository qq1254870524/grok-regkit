...
class Sub2APIClient:
    ...
    def import_grok_sso(
        ...
        *,
        ...
        _log(
            self.log_callback,
            f"[*] Sub2API 入池开始 email={name} endpoint=/api/v1/admin/grok/sso-to-oauth groups={groups}",
        )
        for attempt in (1, 2):
            ...
            if failed:
                ...
                raise RuntimeError(f"Sub2API SSO→OAuth 转换失败: {detail}")
            raise RuntimeError("Sub2API 入池返回 created/failed 均为空")
