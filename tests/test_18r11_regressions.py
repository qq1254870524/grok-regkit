...
class ConsentDiscoveryTests(unittest.TestCase):
    def test_extended_scan_finds_action_after_fast_limit(self):
        m=load_file('sso18r11', ROOT/'sso_to_auth_json.py')
        action='a'*40
        ...
        logs=[]
        ids=m._discover_action_ids_from_js(Session(chunks), html, log=logs.append)
        self.assertIn(action, ids)
        self.assertTrue(any('expand scan' in x for x in logs))
