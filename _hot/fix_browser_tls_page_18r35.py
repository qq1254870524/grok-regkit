...
# fill_email: inject page resolve before first filled = page.run_js with const email
...
# fill_code loop - first occurrence after fill_code def only
...
"""    deadline = time.time() + timeout
    while time.time() < deadline:
        ...
        try:
            clicked = page.run_js(r\"\"\"
""",
"click_email_refresh_page")

idx_c = text.find("def click_email_signup_button")
idx_n = text.find("def page_is_cloudflare_challenge")
if idx_n < 0:
    idx_n = text.find("def wait_cloudflare_passthrough")
seg = text[idx_c:idx_n]
m4 = re.search(r"(try:\n            clicked = page\.run_js\(r\"\"\"(?:.|\n)*?\n        \)\n)", seg)
if not m4:
    raise SystemExit("click wrap search failed")
wrapped = m4.group(1) + """        except Exception as _ce_exc:
            em = str(_ce_exc)
            if (
                ("NoneType" in em and "run_js" in em)
                or "页面被刷新" in em
            ...
            raise
"""
text = text[:idx_c] + seg[:m4.start()] + wrapped + seg[m4.end():] + text[idx_n:]
print("OK click_email try/except")
