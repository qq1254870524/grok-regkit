...
- 2026-07-18r14: full matrix hybrid/browser x direct/socks5 x outlook/aol
  x 10 rounds + pending_sso recovery; classify failures; no secret dumps.
"""
...
OUT = BASE / "matrix_runs" / datetime.now().strftime("matrix_18r14_%Y%m%d_%H%M%S")
...
def classify(logs: str) -> str:
    ...
    rules = [
        ...
        ("create_email_fail", ["createemail", "create email", "发信失败", "发送到此邮箱的验证码过多", "too many"]),
...
def run_one(cell: dict, round_i: int) -> dict:
    ...
    try:
        ...
        # redact secrets-ish patterns
        logs = re.sub(r"(sso[=:\s]+)[A-Za-z0-9_\-\.]{8,}", r"\1***", logs, flags=re.I)
        logs = re.sub(r"(password[=:\s]+)\S+", r"\1***", logs, flags=re.I)
...
def write_report(rows: list[dict]) -> None:
    ...
    for k, v in total.most_common():
        lines.append(f"- `{k}`: {v}")
    lines += ["", "## Notes", "", "- Logs under this directory are redacted for SSO/password-ish patterns.", ""]
    REPORT.write_text("\n".join(lines) + "\n", encoding="utf-8")
