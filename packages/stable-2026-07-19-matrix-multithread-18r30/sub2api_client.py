...
def import_after_success_prefer_cpa(
    ...
    *,
    ...
    This closes the gap where G2A accepts raw SSO but Sub2API device-flow conversion
    hits 429 / GROK_SSO_* while CPA already holds OAuth tokens.
    """
    cfg = config or {}
        ...
        if cpa_path is None:
            ...
            for k in ("path", "auth_path", "file"):
                ...
                if v and Path(str(v)).is_file():
                    ...
                    break
    if cpa_path is None:
        cpa_path = _find_cpa_file_for_email(em, cfg)
    ...
    if cpa_path is not None:
        ...
        try:
            ...
            errors.append(f"cpa_oauth:{exc}")
            _log(log_callback, f"[!] Sub2API CPA OAuth 导入失败，回退 SSO→OAuth: {exc}")

    _log(log_callback, f"[*] Sub2API 回退 SSO→OAuth 导入 email={em}")
    try:
