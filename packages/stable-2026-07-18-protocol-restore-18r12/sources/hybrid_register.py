        ...
        with BrowserTokenSession(log=log) as browser:
            ...
            if not sso:
                ...
                if email and password and not signup_confirmed:
                    try:
                        unconf_line = f"{email}----{password}----unconfirmed:{ui_reason or 'no_sso'}"
                        unconf_fixed = ROOT / "accounts_signup_unconfirmed.txt"
                        with unconf_fixed.open("a", encoding="utf-8") as pf:
                            pf.write(unconf_line + "\n")
                        stamp = time.strftime("%Y%m%d_%H%M%S")
                        unconf_stamp = ROOT / f"accounts_signup_unconfirmed_{stamp}.txt"
                        with unconf_stamp.open("a", encoding="utf-8") as pf:
                            pf.write(unconf_line + "\n")
                        log(
                            f"[hybrid] signup unconfirmed (no SSO); saved to {unconf_fixed.name} "
                            f"and {unconf_stamp.name}; keep mailbox in pool email={email}"
                        )
                    except Exception as unconf_exc:
                        log(f"[hybrid] save unconfirmed account fail: {unconf_exc}")
                    log(
                        f"[hybrid] no sso and not registered; fail (not pending_sso) "
                        f"ui_reason={ui_reason!r} email={email}"
                    )
                    return _result(STATUS_FAIL, email=email, detail=f"signup_unconfirmed:{ui_reason or 'no_sso'}")
                # Confirmed registration without SSO: keep email+password for later OOS/SSO.
                try:
                ...
                # Confirmed signup burned mailbox; remove from pool.
                try:
