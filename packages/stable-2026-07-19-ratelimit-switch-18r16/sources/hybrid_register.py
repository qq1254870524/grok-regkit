...
def save_to_pending_sso_file(
    ...
    """Append email----password----reason to accounts_registered_pending_sso.txt (and stamped copy)."""
    em = (email or "").strip()
...
def burn_mailbox_to_pending(
    ...
    save_to_pending_sso_file(email, password, reason=reason, log=log)
    try:
        remove_mailbox_from_pool(email, reason=reason, log=log)
        if log:
            log(f"[hybrid] mailbox burned to pending_sso email={email} reason={reason}")
    except Exception as rm_exc:
        if log:
        ...
        with BrowserTokenSession(log=log) as browser:
            ...
            if not clean:
                ...
                burn_mailbox_to_pending(
                    email,
                    password,
                    reason="code_timeout_or_empty",
                    log=log,
                )
                return _result(
                    STATUS_PENDING_SSO,
                    email=email,
                    detail="code_timeout_or_empty",
                )
    ...
    try:
        ...
        while i < count:
            ...
            if status == STATUS_SUCCESS:
                ...
                # do not increment i — keep trying until success or other terminal
                log(
