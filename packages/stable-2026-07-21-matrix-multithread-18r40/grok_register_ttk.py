...
def create_browser_options(browser_proxy="", force_headless=None):
    ...
    if sys.platform != "win32":
        ...
        return options
    # 18r31d: Windows smaller headed window (NOT headless — CF/Turnstile needs real chrome)
    try:
...
"""
        ...
        # 18r40: second pass — any chrome/msedge whose CommandLine contains project .chrome-data
        try:
            ps2 = r"""
...
Get-CimInstance Win32_Process -ErrorAction SilentlyContinue |
  Where-Object {
    $_.Name -match '^(chrome|msedge|chromedriver)\.exe$' -and
    $_.CommandLine -and ($_.CommandLine -like ('*' + $root + '*') -or $_.CommandLine -match 'run-\d+-\d+-[0-9a-f]{4}')
  } |
  ForEach-Object {
    try {
      Stop-Process -Id $_.ProcessId -Force -ErrorAction Stop
  ...
  document.querySelectorAll('[data-sitekey], .cf-turnstile, #cf-turnstile').forEach((n) => {
    ...
    try { n.dispatchEvent(new MouseEvent('click', {bubbles:true})); } catch(e) {}
            ...
            if log_callback and attempt % 10 == 4:
                try:
                    log_callback(f"[Debug] Turnstile hard-click pass attempt={attempt+1}/36")
                except Exception:
                    pass
        sleep_with_cancel(1, cancel_callback)
        ...
        for mail_try in range(1, max_mail_retry + 1):
            ...
            try:
                ...
                try:
                    ...
                    pass
        if not mail_ok:
            # 18r30c: align serial — burn last mailbox to pending on code-stage exhaustion
            if email:
                try:
                    from hybrid_register import burn_mailbox_to_pending
                    _pw = ""
                    ...
                    burn_mailbox_to_pending(
                        email,
                        _pw,
                        reason="browser_mt_code_fail",
                        log=wlog,
                        mail_token=str(dev_token or ""),
                    )
                    wlog(
                        f"[!] browser/mt mail fail -> pending_sso+del pool "
                        f"email={email} reason=browser_mt_code_fail"
                    )
                except Exception as pend_exc:
                    wlog(f"[!] browser/mt burn pending fail email={email}: {pend_exc}")
            raise Exception(
                f"pending_sso:browser_code_fail email={email or 'multiple'} 验证码阶段失败"
            )
        wlog(f"[*] 验证码: {code}")
        wlog("[*] 4. 填写资料")
