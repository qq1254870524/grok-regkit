...
> 以本机长期实跑为准。完整还原点见 [`STABLE_VERSION.md`](./STABLE_VERSION.md) / [`CHANGELOG.md`](./CHANGELOG.md)。

| 能力 | 说明 |
|------|------|
| 邮箱来源 | 临时邮（CF/Duck/YYDS）+ **微软 Outlook（Graph）** + **AOL（IMAP+TOTP）** 账号池 |
| 邮箱路由 | 按域名/token 自动选 provider（全局配 AOL 时 Outlook 仍走 Graph） |
| 主路径 | **注册成功 → 立即 SSO → 入池**（g2a / Sub2API / CPA）；`pending_sso` 仅兜底 |
| 二次补 SSO | Web「二次补」/ `POST /api/pending-sso/recover`；登录带 **Turnstile**；**只点一次登录**，失败立即 hybrid 重注册 |
| 代理 | 直连 / 单代理 / **SOCKS5 代理池**（可在 Web 页保存） |
| 注册模式 | `hybrid`（协议优先）· `browser`（全浏览器） |
| 入池 | SSO 会话、Sub2API `sso-to-oauth`、CPA `xai-*.json` OIDC、可选 NSFW |
| 控制台 | `http://127.0.0.1:8092`；应用日志不脱敏 |
| 停止策略 | 「停止注册」只停 grok-regkit 任务；**不**停 Sub2API / g2a / CLIProxy / CPA Gateway |

### 推荐还原点

| 用途 | Tag |
|------|-----|
| 文档+全关联同步（当前） | `stable-2026-07-19-docs-sync-18r28i` |
| pending 禁二次登录业务完美点 | `stable-2026-07-19-pending-one-login-18r28h` |
...
本地 Packages：`packages/stable-2026-07-19-*.zip`（历史包全部保留）。

### 关联公开仓库

| 项目 | URL |
|------|-----|
| 本仓 grok-regkit | https://github.com/qq1254870524/grok-regkit |
| 服务编排 | https://github.com/qq1254870524/grok-regkit-services |
| Sub2API fork | https://github.com/qq1254870524/sub2api |
| grok2api | https://github.com/qq1254870524/grok2api |
| Turnstile 独立抽取 | https://github.com/qq1254870524/turnstile-harvester1 |
