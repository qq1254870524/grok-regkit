...
class AolAccount:
    ...
    password: str = ""
    totp_secret: str = ""
    status: str = "idle"  # idle | warming | in_use | bad | registered
...
class AolImapSession:
    ...
    def fetch_recent(self, top_per_folder: int = 5) -> List[dict]:
...
class AolAccountPool:
    ...
    def acquire(self) -> Tuple[str, str]:
        """Claim one AOL mailbox exclusively for a worker (18r41 reserve-before-login)."""
        last_err = None
        with _POOL_LOCK:
        ...
        while attempts < max_attempts:
            ...
            with _POOL_LOCK:
                ...
                while scanned < n:
                    ...
                    if st in ("bad", "registered", "in_use", "warming") or acc.cooldown_until > _now():
                        ...
                        continue
                    self._lg(f"[*] AOL acquire reserve: {acc.email} | pool={n} | auth=IMAP password/app-password")
                    acc.status = "in_use"
            ...
            if reserved is None:
                continue
            try:
                self.ensure_login(reserved)
                token_blob = json.dumps(
                    {
                        "email": reserved.email,
                        "password": reserved.password,
                        "totp_secret": reserved.totp_secret,
                        "provider": "aol",
                        "protocol": "imap",
                    },
                    ensure_ascii=False,
                )
                self._lg(f"[+] AOL ready: {reserved.email} | protocol=IMAP imap.aol.com:993")
                ...
                self._lg(format_aol_login_error(reserved.email, exc, stage="acquire"))
                with _POOL_LOCK:
                    still = None
                    ...
                    if auth_fail:
                        ...
                        self._lg(
                            f"[!] AOL 临时失败，冷却 120s | email={reserved.email} "
                            f"category={info.get('category')} "
                            f"exc={info.get('exception_type')} raw={info.get('raw_error')}"
                        )
                continue
        if last_err:
            raise Exception(f"AOL acquire failed after retries: {last_err}")
        raise Exception("AOL account pool exhausted (all in_use/warming/cooldown/bad)")
...
def preflight_mailbox(config, token_blob: str, email: str, *, log_callback=None, top: int = 10) -> dict:
    """18r30: AUTH-ONLY preflight (IMAP login). Full folder top=N scan happens at code poll time."""
    pool = get_pool(config, log_callback=log_callback)
    ...
    _log(
        ...
        f"mode=auth_only (poll still ALL folders top={int(top)})",
    )
    ...
    try:
        ...
        summary = {
            ...
            "total": 0,
            "folder_counts": {},
            "scanned_folders": "auth_only",
            "top": int(top),
            "full_mailbox": False,
        }
        _log(log_callback, f"[+] AOL preflight OK email={em} mode=auth_only")
        return summary
    finally:
        try:
            sess.close()
