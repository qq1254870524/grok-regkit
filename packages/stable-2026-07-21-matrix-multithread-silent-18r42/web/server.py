...
def _update_job_progress_from_log(message: str) -> None:
    ...
    if "starting registration" in low or "混合任务启动" in text or "混合模式启动" in text or "开始第" in text:
        ...
        # keep live counters only. Stats lines always contain the word pending_sso
        # (e.g. "pending_sso 45") — that must NOT flip phase to recovery.
    ...
    if mpend and ("pending_sso saved" in text or "mailbox burned to pending_sso" in text):
        # require email key — full log text as key inflates pending to ~100 spuriously
        em = (mpend.group(1) or "").strip().lower()
        if not em:
            return
        key = em
        with _job_lock:
            if key not in _progress_seen_pending:
                ...
                _job_state["pending_sso"] = int(_job_state.get("pending_sso") or 0) + 1
...
class ConfigBody(BaseModel):
    ...
    email_preflight_limit: Optional[int] = None
    email_preflight_continuous: Optional[bool] = None
    email_preflight_warm_ahead: Optional[int] = None
    email_preflight_warm_ttl_sec: Optional[float] = None
    email_preflight_interval_sec: Optional[float] = None
    ...
    # 18r42 silent multi-thread browser (headed but minimize/offscreen)
    browser_silent: Optional[bool] = None
    browser_start_minimized: Optional[bool] = None
    browser_window_x: Optional[int] = None
    browser_window_y: Optional[int] = None
    browser_window_width: Optional[int] = None
    browser_window_height: Optional[int] = None
...
async def api_put_config(body: ConfigBody, x_access_key: Optional[str] = Header(None)):
    ...
    for key, value in updates.items():
        if key in SECRET_FIELDS and isinstance(value, str):
            ...
            # Empty string: keep previous secret (avoid matrix/UI partial save wiping credentials).
            # Explicit clear only via value "__CLEAR__".
            if stripped == "":
                continue
            if stripped == "__CLEAR__":
                engine.config[key] = ""
                continue
...
async def api_status(x_access_key: Optional[str] = Header(None)):
    ...
    try:
        ...
        try:
            ...
            awaiting = max(int(pending or 0), int(unfinished or 0))
        except Exception:
...
@app.post("/api/sub2api/reconcile")
async def api_sub2api_reconcile(x_access_key: Optional[str] = Header(None)):
    """Reconcile G2A/hybrid/CPA into Sub2API (missing only)."""
    _require_auth(x_access_key)
    cfg = engine.config
    try:
        from sub2api_client import reconcile_sub2api_pools, log_pool_counts
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"sub2api_client unavailable: {exc}") from exc
    ...
    def _run():
        ...
        summary = reconcile_sub2api_pools(config=cfg, log_callback=_log, limit=0)
        counts = log_pool_counts(config=cfg, log_callback=_log, email="api_reconcile")
        return {"ok": True, "summary": summary, "counts": counts, "logs": logs[-80:]}
...
async def api_pending_sso_recover(
    ...
    with _job_lock:
        ...
        workers = max(1, min(32, int(getattr(body, "workers", 1) or 1)))
        _append_log(f"[*] starting pending_sso recovery count={count or 'all'} workers={workers}")
        t = threading.Thread(
            target=_run_job,
            args=(count,),
            kwargs={"job_kind": "pending_sso_recovery", "workers": workers},
            daemon=True,
        )
        _job_thread = t
        t.start()
...
async def api_accounts_list(x_access_key: Optional[str] = Header(None)):
    ...
    pending = {
        ...
        "format": "email----password----reason----b64:mail_token",
        "importable_sso": False,
        "note": "pending 文件第4段是邮箱 mail_token（Outlook/AOL），不是 Grok session SSO，不能直接导入 G2A/Sub2API；请用二次补SSO成功后的 accounts_reregistered_* / accounts_importable_sso_*",
        "timestamped": [
...
@app.post("/api/accounts/export-importable-sso")
async def api_accounts_export_importable_sso(x_access_key: Optional[str] = Header(None)):
    """Merge recovered/hybrid session SSO into accounts_importable_sso_*.txt (no mail_token)."""
    _require_auth(x_access_key)

    def _run():
        from protocol.export_importable_sso import write_importable_sso_export

        return write_importable_sso_export(ROOT)

    meta = await asyncio.to_thread(_run)
    try:
        _append_log(
            f"[+] export importable SSO file={meta.get('out_file')} "
            f"unique={meta.get('unique_emails')} skipped_mail_token={meta.get('skipped_mail_token')}"
        )
    except Exception:
        pass
    return {"ok": True, **meta}
