## 2026-07-21 / 18r42 silent multi-thread (w=10 n=100 pre=20 SOCKS5)

- **Silent headed browser** (NOT pure headless — CF/Turnstile still works):
  - config: `browser_silent`, `browser_start_minimized`, `browser_window_x/y` (default off-screen -32000)
  - Chrome flags: `--start-minimized` + off-screen position
  - post-launch Win32 `SW_SHOWMINNOACTIVE` (no focus steal)
- Web ConfigBody accepts silent keys; matrix writes config.json + `/api/config`
- Matrix: `tools/matrix_18r42_silent_mt.py` — hybrid/browser × socks5 × outlook/aol ×2 + pending_sso ×2 + stop ×2
- Params: workers=10, count=100, preheat=20; hidden process (no Python console)
- Package tag planned: `stable-2026-07-21-matrix-multithread-silent-18r42` (**no overwrite** 18r41/18r40)
