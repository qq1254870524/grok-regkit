...
def _iter_idle_accounts(pool: Any) -> List[Any]:
    ...
    for acc in accounts:
        status = str(getattr(acc, "status", "") or "")
        if status in ("bad", "registered", "in_use"):
            continue
        email = str(getattr(acc, "email", "") or "").strip()
        if not email:
            continue
...
def _preflight_one_account(
    *,
    ...
    """Return (ok, email). On permanent fail removes from pool."""
    email = str(getattr(acc, "email", "") or "").strip()
    if not email:
        return False, ""
    c = config if isinstance(config, dict) else {}
    ...
    try:
        ...
        log(
            ...
            f"exc={type(exc).__name__}: {exc} | remove from pool"
        )
        try:
            if provider == "aol":
                import aol_mail as am
...
class ContinuousEmailPreflight:
    ...
    def _pick_next(self, provider: str) -> Optional[Any]:
        ...
        if not idle:
            return None
        # prefer not-warm accounts; rotate cursor
        n = len(idle)
