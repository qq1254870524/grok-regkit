...
def classify(logs: str, st: dict) -> str:
    ...
    # Status counters always win. Do NOT match bare "pending_sso" in log text —
    # snapshots always contain that key name and mis-label real successes.
    ...
    if p > 0:
        return "pending_sso"
    # real pending markers only (not the json field name alone)
    if any(
        k in t
        for k in (
            "[pending_sso]",
            ...
            "写入 pending",
            "saved pending_sso",
            "accounts_registered_pending_sso",
            "no sso yet",
            "等待二次补SSO",
        )
    ):
...
def run_register_or_pending(cell: dict, round_i: int) -> dict:
    ...
    try:
        ...
        if rec["class"] == "pool_import_fail":
            ...
            rec["ok"] = True
            # prefer counter-based class over noisy log substring class
            if rec.get("pending_sso", 0) == 0:
                rec["class"] = "success" if int(rec.get("fail") or 0) == 0 else "success_partial"
