...
def main() -> int:
    ...
    accounts = parse_accounts_file(args.accounts, require_session_sso=True)
    if args.email:
    ...
    print(f"accounts total={len(parse_accounts_file(args.accounts, require_session_sso=True))} todo={len(todo)} out={args.out_dir}")
