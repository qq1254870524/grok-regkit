...
def sso_to_token(sso_cookie: str, proxy: str = "", proxy_candidates=None, log=print, allow_direct_fallback: bool = True) -> dict | None:
    ...
    # Build route list: candidates + optional true direct (empty proxy)
    routes = []
    seen = set()
    for raw in [proxy, *(list(proxy_candidates or []))]:
        p = (str(raw) if raw is not None else "").strip()
        if not p or p.lower() in {"direct", "none", "off", "false", "0", "__direct__"}:
            continue
        if p in seen:
            continue
        seen.add(p)
    ...
    if allow_direct_fallback:
        routes.append("")  # true direct last

    last_net_err = None
    s = None
    r = None
    for ri, route in enumerate(routes or [""], 1):
        label = route if route else "direct"
        # redact password in log if any
        try:
            from urllib.parse import urlparse as _up
            if route:
                u = _up(route if "://" in route else f"socks5h://{route}")
                label = f"{u.scheme}://{u.hostname}:{u.port}" if u.hostname else route[:40]
        except Exception:
            label = (route or "direct")[:60]
        log(f"  [*] authcode route {ri}/{len(routes or [''])} {label}")
        s = requests.Session()
        if route:
            s.proxies = {"http": route, "https": route}
        else:
            s.proxies = {}
        ...
        try:
            ...
            log(f"  ⚠ authcode route fail {label}: {e}")
            if transport and ri < len(routes or [""]):
                continue
            if ri < len(routes or [""]):
                continue
            log(f"  ❌ 网络错误: {e}")
