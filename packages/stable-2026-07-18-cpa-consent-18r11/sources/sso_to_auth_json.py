...
def _discover_action_ids_from_js(session, html: str, base_url: str = "https://accounts.x.ai", log=None) -> list[str]:
    ...
    18r11 uses a fast first phase, then expands only when no usable action id was
    extracted.  A chunk containing consent/allow words is not itself success:
    ...
    fast_limit, total_limit = 12, 40
    fast_budget, total_budget = 10.0, 28.0
    per_timeout = 8
    fetched = 0
    strong_ids = 0
            ...
            for regex in (_CREATE_SERVER_REF_RE, _CALL_SERVER_RE):
                for match in regex.finditer(text):
                    ...
                    if _add(match.group(1), prefer=prefer):
                        ...
                        if prefer:
                            strong_ids += 1
            if phase == "fast" and strong_ids > 0 and added_here > 0 and fetched >= 3:
                break

    _scan_until(fast_limit, fast_budget, "fast")
    if not priority and not found and fetched < total_limit and (time.time() - t0) < total_budget:
        if log:
            log(f"  [*] consent JS fast phase no action id; expand scan from {fetched} to max {total_limit}")
        _scan_until(total_limit, total_budget, "extended")
    ...
    if log:
        log(f"  [*] 从 JS chunks 解析 Next-Action {len(ordered)} 个（扫 {fetched} 个脚本, elapsed={time.time()-t0:.1f}s, strong_ids={strong_ids}）")
    return ordered
