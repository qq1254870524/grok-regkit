        ...
        with BrowserTokenSession(log=log) as browser:
            ...
            for _try in range(20):
                ...
                try:
                    ...
                    # 18r31c: skip re-login if continuous/start preflight already warm
                    try:
                        from worker_coord import is_preflight_warm
                        _ttl = float((_cfg_pre or {}).get("email_preflight_warm_ttl_sec") or 600)
                        if is_preflight_warm(email, ttl_sec=_ttl):
                            log(
                                f"[hybrid] mailbox preflight SKIP (warm) email={email} "
                                f"ttl={_ttl}s — continuous pre-login already OK"
                            )
                            break
                    except Exception as warm_exc:
                        log(f"[hybrid] warm check skip-fail: {warm_exc}")
                    ...
                    if is_aol:
                        ...
                        try:
                            ...
                            from worker_coord import mark_preflight_warm
                    ...
                    try:
                        ...
                        from worker_coord import invalidate_preflight_warm
...
def run_hybrid_registration_job(count, log_callback=None, controller=None, workers=None):
    ...
    # ---- 18r30 multi-thread ----
    # 18r31c: continuous background email pre-login warm queue
    ...
    if bool(engine.config.get("email_preflight_on_start", True)):
        ...
        try:
            ...
            start_continuous_preflight(engine.config, log=log, top=int(engine.config.get("mail_top_per_folder") or 5))
...
def _run_hybrid_registration_job_mt(
    ...
    *,
    ...
    try:
        ...
        log(f"[!] stop continuous preflight: {_scp}")
