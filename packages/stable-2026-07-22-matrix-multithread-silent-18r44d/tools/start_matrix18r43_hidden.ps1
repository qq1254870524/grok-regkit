...
# 18r43e: skip launch if matrix already alive; resume-capable matrix script
...
$existing = Get-CimInstance Win32_Process | Where-Object { $_.CommandLine -match 'matrix_18r43_silent_stable_mt' }
if ($existing) {
  $pidM = @($existing)[0].ProcessId
  Set-Content -LiteralPath (Join-Path $LogDir '_matrix_18r43.pid') -Value $pidM -Encoding ascii
  Write-Host "18r43 matrix already running pid=$pidM (no re-launch)"
  exit 0
}
