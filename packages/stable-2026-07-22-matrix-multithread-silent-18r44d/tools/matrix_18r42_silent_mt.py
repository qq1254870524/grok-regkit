# -*- coding: utf-8 -*-
"""18r42 silent multi-thread matrix (workers=10, preheat=20, count=100).
...
ROUNDS_PER_CELL = 2
PREHEAT = 20
# 100 accounts / 10 workers ~ long; allow up to 6h per cell
TIMEOUT_PER_JOB = 6 * 60 * 60
    ...
    while time.time() - t0 < timeout:
        ...
        if now - last_print >= 60:
            ...
            print(
                ...
                f"pending={st.get('pending_sso')} phase={st.get('phase')} "
...
def main():
    ...
    jsonl = OUT / f"matrix_18r42_{stamp}.jsonl"
    summary_path = OUT / f"matrix_18r42_{stamp}_summary.json"
    report_path = OUT / f"MATRIX_18r42_{stamp}.md"
    board = OUT / "_CODEX_MATRIX_18r42_BOARD.md"
    ...
    print(
        f"18r42 silent matrix start workers={WORKERS} count={COUNT} preheat={PREHEAT} "
    ...
    for idx, item in enumerate(cells, 1):
        ...
        # live board
        lines = [
            f"# Matrix 18r42 silent MT board",
    ...
    md = [
        f"# Matrix 18r42 silent multi-thread",
        f"stamp={stamp}",
        f"workers={WORKERS}",
        f"count={COUNT}",
        f"preheat={PREHEAT}",
