...
def bind_worker_proxy(engine, worker_id: int, log: Optional[LogFn] = None) -> str:
    ...
    SOCKS5 list: prefer pool[(worker_id-1) % n], but only bind LIVE nodes (18r44d precheck).
    Other modes: use config['proxy'] / resolve once; optional quick_check when available.
    ...
    try:
        if hasattr(engine, "is_socks5_list_mode") and engine.is_socks5_list_mode(c):
            ...
            if pool:
                prefer_idx = (wid - 1) % len(pool)
                prefer = pool[prefer_idx]
                # 18r44d: only bind LIVE SOCKS5 — TCP+HTTP precheck before browser
                live = ""
                if hasattr(engine, "pick_live_proxy"):
                    try:
                        live = str(
                            engine.pick_live_proxy(
                                c,
                                log_callback=lg,
                                prefer_url=prefer,
                                start_index=prefer_idx,
                                max_try=min(len(pool), 8),
                                timeout=float(c.get("proxy_precheck_timeout", 8) or 8),
                            )
                            or ""
                        ).strip()
                    except Exception as pe:
                        lg(f"[!] worker proxy live pick fail worker={wid}: {pe}")
                        live = ""
                if live:
                    proxy = live
                    lg(
                        f"[*] worker proxy bind socks5_list LIVE worker={wid} "
                        f"prefer_index={prefer_idx}/{len(pool)} | {proxy}"
                    )
                else:
                    proxy = prefer
                    lg(
                        f"[!] worker proxy bind socks5_list no LIVE node; use prefer "
                        f"worker={wid} index={prefer_idx}/{len(pool)} | {proxy}"
                    )
            else:
            ...
            if proxy and hasattr(engine, "quick_check_proxy") and mode not in ("direct", "none", "off", ""):
                try:
                    chk = engine.quick_check_proxy(
                        proxy,
                        timeout=float(c.get("proxy_precheck_timeout", 8) or 8),
                        log_callback=lg,
                    )
                    if not (isinstance(chk, dict) and chk.get("ok")):
                        lg(
                            f"[!] worker proxy precheck fail worker={wid} | "
                            f"{(chk or {}).get('detail') if isinstance(chk, dict) else chk}"
                        )
                except Exception as pe:
                    lg(f"[!] worker proxy precheck error worker={wid}: {pe}")
